...
class QualityCheckWizard(models.TransientModel):
    ...
    def action_generate_next_window(self):
        ...
        if picking_ids_to_validate:
            ...
            if validate_action is not True and validate_action.get('xml_id') != 'esl_quality_control.action_quality_check_wizard':
    ...
    def show_failure_message(self):
        ...
        return {
            ...
            'views': [(self.env.ref('esl_quality_control.quality_check_wizard_form_failure').id, 'form')],
            'target': 'new',
            'res_id': self.id,
            'context': {
                **self.env.context,
